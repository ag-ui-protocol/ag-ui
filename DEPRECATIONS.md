...
| `parentMessageId: null` on `TOOL_CALL_START`                                                | omit the field                                                                                                            | inbound boundary (TS); model validation (Python); converters (.NET) | 2027-08-24 |
| `parentMessageId: null` on `TOOL_CALL_CHUNK`                                                | omit the field                                                                                                            | inbound boundary (TS); model validation (Python); converters (.NET) | 2027-08-24 |
| `outcome: null` on `RUN_FINISHED`                                                           | omit the field                                                                                                            | inbound boundary (TS); model validation (Python); converters (.NET) | 2027-08-24 |
| `rawEvent: null` on an event                                                                | omit the field                                                                                                            | inbound boundary (TS); model validation (Python); converters (.NET) | 2027-09-08 |
| `result: null` on `RUN_FINISHED`                                                            | omit the field                                                                                                            | inbound boundary (TS); model validation (Python); converters (.NET) | 2027-09-08 |
| `result: null` on `SUBAGENT_FINISHED`                                                       | omit the field                                                                                                            | inbound boundary (TS); model validation (Python); converters (.NET) | 2027-09-08 |
| `payload: null` on a resume entry                                                           | omit the field                                                                                                            | inbound boundary (TS, events); model validation (Python); converters (.NET) | 2027-09-08 |
| `metadata: null` on an `image` content part                                                 | omit the field                                                                                                            | inbound boundary (TS, events); model validation (Python); converters (.NET) | 2027-09-08 |
| `metadata: null` on an `audio` content part                                                 | omit the field                                                                                                            | inbound boundary (TS, events); model validation (Python); converters (.NET) | 2027-09-08 |
| `metadata: null` on a `video` content part                                                  | omit the field                                                                                                            | inbound boundary (TS, events); model validation (Python); converters (.NET) | 2027-09-08 |
| `metadata: null` on a `document` content part                                               | omit the field                                                                                                            | inbound boundary (TS, events); model validation (Python); converters (.NET) | 2027-09-08 |
| `parameters: null` on a tool                                                                | omit the field                                                                                                            | inbound boundary (TS, events); model validation (Python); converters (.NET) | 2027-09-08 |
| `forwardedProps: null` on `RunAgentInput`                                                   | omit the field                                                                                                            | inbound boundary (TS, events); model validation (Python); converters (.NET) | 2027-09-08 |
...
All three SDKs accept the listed nulls, under the same rows and the same
expiry dates. In TypeScript the conversion runs at the client's inbound event
boundary described above; in Python it runs during model validation, so
anything parsing an event or a `RunAgentInput` through the generated models
gets it; in .NET it runs in the JSON converters, on read. Each of them emits a
deprecation warning per occurrence — this is a shim, not a supported shape —
and each stops at the expiry in the row, after which the `null` is malformed
