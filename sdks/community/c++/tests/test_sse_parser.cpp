...
TEST_CASE(GetLastError) {
    ...
    // Feed valid SSE format with invalid JSON content
    parser.feed("data: {invalid}\n\n");
    
    // SSE parsing succeeds, event is created
    ASSERT_TRUE(parser.hasEvent());
    
    // SSE parser doesn't validate JSON, so no error is set
    EXPECT_EQ(parser.getLastError(), "");
    
    // JSON parsing is upper layer's responsibility
    auto jsonStr = parser.nextEvent();
    bool parseError = false;
    try {
        nlohmann::json::parse(jsonStr);
