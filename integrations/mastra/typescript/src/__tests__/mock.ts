...
export function collectEvents(
  ...
  input: RunAgentInput,
  options: { mergeTextChunks?: boolean } = {},
): Promise<BaseEvent[]> {
  return new Promise((resolve, reject) => {
    const events: BaseEvent[] = [];
    let mergedChunk: (BaseEvent & { delta?: string }) | null = null;
    adapter.run(input).subscribe({
      next: (e) => {
        if (options.mergeTextChunks && e.type === EventType.TEXT_MESSAGE_CHUNK) {
          const chunk = e as BaseEvent & { delta?: string };
          if (!mergedChunk) {
            mergedChunk = { ...chunk };
          } else {
            mergedChunk.delta = (mergedChunk.delta ?? '') + (chunk.delta ?? '');
          }
          return;
        }
        events.push(e);
      ...
      error: reject,
      complete: () => {
        if (mergedChunk) events.splice(events.length - 1, 0, mergedChunk);
        resolve(events);
      },
    });
  });
...
export type WriteTestJsonOutputOptions = {
  fromImportMetaUrl: string;
  filePrefix: string;
  payload: unknown;
};

export async function writeTestJsonOutput(options: WriteTestJsonOutputOptions): Promise<string> {
  const testsDir = dirname(fileURLToPath(options.fromImportMetaUrl));
  const tmpDir = join(testsDir, '.tmp');
  const outputFilePath = join(tmpDir, `${options.filePrefix}.json`);

  await mkdir(dirname(outputFilePath), { recursive: true });
  await Bun.write(outputFilePath, JSON.stringify(options.payload, null, 2));
