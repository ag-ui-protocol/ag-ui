...
python/       keyless example server driving CAO's `mock_cli` fleet through
              run_plane_stream, exposing four Dojo features on port 8027:
...
The example server ships two keyless backends for the same four features,
...
deliberately stays the thin **L1/L2 consumer + the four standard Dojo features** (agentic_chat,
shared_state, human_in_the_loop, interrupt): no new protocol code and no additions to the frozen
