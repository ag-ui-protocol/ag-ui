...
describe("Orchestrator path", () => {
  ...
  it("failed node emits STEP_FINISHED and surfaces no error event", async () => {
    ...
    // translates nor logs it, so a node failure leaves no trace at all. What
    // does end an orchestrator run is a throw out of the stream (see
    // orchestrator-stop-reasons.test.ts).
    const stream = fakeOrchestrator([
