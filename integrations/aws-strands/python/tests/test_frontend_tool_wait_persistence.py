...
@pytest.mark.asyncio
async def test_cached_wrapper_refreshes_partial_wait_written_by_another_wrapper(
    tmp_path: Path,
):
    thread_id = "load-balanced-partial-refresh"
    model = _PersistenceModel(("first", "second"))
    tools = [_tool("first"), _tool("second")]
    wrapper_a = _adapter(model, tools, tmp_path, thread_id)
    first = await _collect(
        wrapper_a,
        _input(
            thread_id,
    ...
    assert "'text': 'one'" in transcript
    assert "'text': 'two'" in transcript


@pytest.mark.asyncio
async def test_cached_wrapper_accepts_new_turn_after_other_wrapper_consumes_wait(
    tmp_path: Path,
):
    thread_id = "load-balanced-consumed-refresh"
    model = _PersistenceModel()
    tools = [_tool("lookup")]
    wrapper_a = _adapter(model, tools, tmp_path, thread_id)
    first = await _collect(
    ...
    resumed = await _collect(
        ...
        _input(
            ...
            tools,
            run_id="run-2",
            messages=[ToolMessage(id="result", tool_call_id=wire_id, content="one")],
        ),
    )
    assert EventType.RUN_ERROR not in _types(resumed)
    assert model.calls == 2

    next_turn = await _collect(
        wrapper_a,
        _input(
            thread_id,
            tools,
            run_id="run-3",
            messages=[UserMessage(id="user-2", content="what next?")],
        ),
    )

    assert EventType.RUN_ERROR not in _types(next_turn)
    assert EventType.RUN_FINISHED in _types(next_turn)
    assert model.calls == 3
...
@pytest.mark.asyncio
async def test_same_wrapper_retry_reflushes_failed_end_handoff(
    tmp_path: Path,
):
    thread_id = "same-wrapper-reflush-handoff"
    model = _PersistenceModel()
    tools = [_tool("lookup")]
    failure_counter = {"count": 0}
    adapter = _adapter(
    ...
    [wire_id] = _wire_ids(failed)
    assert EventType.TOOL_CALL_END in _types(failed)
    assert EventType.RUN_ERROR in _types(failed)
    assert EventType.RUN_FINISHED not in _types(failed)
    durable_after_failure = load_frontend_tool_wait(
        _restored_core(model, tmp_path, thread_id).state
    )
    assert durable_after_failure.calls[0].end_handed_off is False
    ...
    assert _types(retried) == [
        EventType.RUN_STARTED,
        EventType.TOOL_CALL_END,
        EventType.RUN_FINISHED,
    ]
    assert _end_ids(retried) == [wire_id]
    persisted = load_frontend_tool_wait(
        _restored_core(model, tmp_path, thread_id).state
    )
    assert persisted.calls[0].wire_tool_call_id == wire_id
    assert persisted.calls[0].end_handed_off is True

    fresh = await _collect(
        _adapter(model, tools, tmp_path, thread_id),
        _input(thread_id, tools, run_id="run-3", messages=[]),
    )
