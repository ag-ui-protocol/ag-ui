...
export class ADKEventTranslator {
  ...
  /** True while a sub-agent invocation is still open. */
  hasOpenSubagents(): boolean {
    return this.tracker.hasOpenInvocations();
  }

  /**
   * Closes open text and reasoning streams. Sub-agents are left open:
   * `SubagentFinishedOutcome` is success | suspended, with no cancelled
   * variant.
   */
  closeOpenStreams(): AGUIEvent[] {
