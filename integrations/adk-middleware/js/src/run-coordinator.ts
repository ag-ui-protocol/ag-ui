...
/**
 * Tool call ids on this run's assistant messages with no matching tool
 * message, in call order. Scoped to `emittedMessageIds` so calls carried in
 * from earlier runs are excluded.
 */
function unansweredToolCallIds(
  messages: readonly Message[],
  emittedMessageIds: readonly string[],
): string[] {
  const emitted = new Set(emittedMessageIds);
  ...
  for (const message of messages) {
    if (message.role !== "assistant" || !emitted.has(message.id)) {
      continue;
    }
  ...
  return {
    ...
    outcome:
      outcome.interrupts.length > 0
        ...
        : {
            type: "success",
            ...(outcome.pendingToolCallIds?.length
...
export class ADKRunCoordinator {
  ...
  async execute(
    ...
    try {
      ...
      // Aborted with no sub-agent open: close streams, emit RUN_FINISHED
      // outcome=cancelled. With one open, SubagentFinishedOutcome has no
      // cancelled variant, so fall through to RUN_ERROR.
      const deliberateStop =
        signal.aborted && !(error instanceof ADKJSProtocolError);
      if (deliberateStop && !translator?.hasOpenSubagents()) {
        for (const event of translator?.closeOpenStreams() ?? []) {
          emitAndTrack(event);
        }
        emitAndTrack(runCancelledEvent(input, translator?.getUsage()));
        return;
      }
      throw error;
    } finally {
  ...
  private async finalize(
    ...
    emit(
      runFinishedEvent(input, {
        result,
        interrupts,
        usage,
        pendingToolCallIds: unansweredToolCallIds(
          messages,
          marker.emittedMessageIds,
        ),
      }),
    );
  }
