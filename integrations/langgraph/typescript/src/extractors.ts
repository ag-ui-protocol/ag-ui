...
export function isSubgraphStreamEvent(
...
): event is StreamResponseChunk & {
  method: "lifecycle";
  params: { data: SubgraphStreamEvent };
} {
  // OLD:
  ...
  return (
    event?.method === "lifecycle" && event?.params?.data?.type === "subgraph"
  );
}
...
export function isMessageTupleEvent(
  event: StreamResponseChunk,
): event is StreamResponseChunk & {
  method: "lifecycle";
  params: { data: { type: "message_tuple"; message: any } };
} {
  // OLD:
  ...
  return (
    ...
    event?.params?.data?.type === "message_tuple"
  );
}
