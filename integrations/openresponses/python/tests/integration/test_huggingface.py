"""Integration tests for Hugging Face provider."""
...
@pytest.fixture
def hf_agent() -> OpenResponsesAgent:
    """Create a Hugging Face agent for testing."""
    return OpenResponsesAgent(
        OpenResponsesAgentConfig(
            base_url=os.environ.get(
                "HUGGINGFACE_ENDPOINT", "https://api-inference.huggingface.co/v1"
            ),
            api_key=os.environ.get("HUGGINGFACE_API_KEY", ""),
            default_model=os.environ.get("HUGGINGFACE_MODEL", "meta-llama/Llama-3.1-8B-Instruct"),
        )
    )
...
def simple_input() -> RunAgentInput:
    ...
    return RunAgentInput(
        thread_id="hf-test-thread-1",
        run_id="hf-test-run-1",
        messages=[
            UserMessage(
                id="msg-1",
                role="user",
...
class TestHuggingFaceBasicStreaming:
    """Tests for basic text streaming with Hugging Face."""

    @pytest.mark.asyncio
    async def test_simple_text_response(
        self, hf_agent: OpenResponsesAgent, simple_input: RunAgentInput
    ):
        """Should stream a simple text response."""
        events = []
        async for event in hf_agent.run(simple_input):
            events.append(event)
...
class TestHuggingFaceErrorHandling:
    """Tests for error handling with Hugging Face."""
