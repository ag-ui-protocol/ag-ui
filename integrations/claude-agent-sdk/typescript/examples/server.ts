 ...
 * matching the pattern used by the Python version and other integrations.
 *
 * Adapters are created once as singletons so session state (sessionIdsByThread)
 * persists across requests — essential for multi-turn conversations.
 *
 * Usage:
...
// Import agent factory functions from standalone example files
...
// ---------------------------------------------------------------------------
// Create adapters ONCE (singletons — session state persists across requests)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// HTTP server with SSE streaming (mirrors Python FastAPI server)
// ---------------------------------------------------------------------------
async function handleRequest(
  ...
  if (req.method === "POST" && adapters[path]) {
    ...
    const observable = adapter.run(inputData);
    observable.subscribe({
      ...
      error: (err) => {
        ...
        res.write(encoder.encode(errorEvent as any));
        res.end();
      },
      complete: () => {
        res.end();
