...
describe("projectA2UIHistory", () => {
  ...
  it("reads a result whose operations arrive as content parts", () => {
    const outerCall = {
      id: "outer",
      type: "function" as const,
      function: { name: "generate_a2ui", arguments: "{}" },
    ...
    const durable = (content: ToolMessage["content"]): Message[] => [
      { id: "outer-assistant", role: "assistant", toolCalls: [outerCall] },
      { id: "outer-result", role: "tool", toolCallId: "outer", content },
    ];

    // One payload split across parts. The projector joins them in order before
    // parsing, so it restores exactly what the plain-text form restores.
    const fromParts = activities(
      projectA2UIHistory(
        snapshot(
          durable([
    ...
    expect(fromParts).toHaveLength(1);
    expect(fromParts).toEqual(
      activities(projectA2UIHistory(snapshot(durable(payload))).messages),
    );
  });
