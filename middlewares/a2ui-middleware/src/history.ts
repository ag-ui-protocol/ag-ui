...
export function projectA2UIHistory(
  ...
  for (const [callId, call] of calls) {
    ...
    // A tool result carries either text or content parts. Both reach this
    // projector as one JSON payload, so flatten once and parse the text. A
    // result that is entirely media flattens to "", which parses to undefined
    // and falls through to the same restore path as an unparseable result.
    const text = contentToText(result?.content);
    const value = result ? parse(text) : undefined;
    ...
    let operations =
      result && !result.error
        ? tryParseA2UIOperations(text)?.operations
        : undefined;
