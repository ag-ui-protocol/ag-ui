...
export const rolePrompt = `**Role:** You are an expert Routing Delegator. Your primary function is to accurately delegate user inquiries to the appropriate specialized remote agents.`;
...
**Task Delegation/Reaching Out to Remote Agents**
- Utilize the \`send_message_to_a2a_agent\` function to communicate with and assign actionable tasks to remote agents.
`.trim();
...
const getRoster = (agentCards: AgentCard[]) => `
**Agent Roster:**
* Available Agents:
${JSON.stringify(agentCards.map((agent) => ({ name: agent.name, description: agent.description })), null, 2)}
`.trim();
