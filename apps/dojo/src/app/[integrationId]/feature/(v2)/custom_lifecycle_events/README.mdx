...
Each assistant reply has a compact usage label immediately above it, showing
...
- The page captures the current `runId` from `RUN_STARTED`, associates the
  `run_usage` event with that run in a small external store, then uses
  `CopilotKitProvider`'s stable `renderCustomMessages` configuration to render
  the label above the assistant message
