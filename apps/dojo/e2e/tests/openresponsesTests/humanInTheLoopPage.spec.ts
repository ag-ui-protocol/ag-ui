...
test.describe("Human in the Loop Feature", () => {
  test("[OpenResponses] should interact with the chat and perform steps", async ({
    page,
  }) => {
    await retryOnAIFailure(async () => {
      ...
      await page.goto(
        "/openresponses/feature/human_in_the_loop"
      );
  ...
  test("[OpenResponses] should interact with the chat using predefined prompts and perform steps", async ({
    page,
  }) => {
    await retryOnAIFailure(async () => {
      const humanInLoop = new HumanInLoopPage(page);
