...
test("[Microsoft Agent Framework .NET] A2UI Dynamic Schema renders hotel comparison surface", async ({
  ...
  await page.goto("/microsoft-agent-framework-dotnet/feature/a2ui_dynamic_schema");
...
test("[Microsoft Agent Framework .NET] A2UI Dynamic Schema renders product comparison surface", async ({
  page,
}) => {
  await page.goto("/microsoft-agent-framework-dotnet/feature/a2ui_dynamic_schema");
...
test("[Microsoft Agent Framework .NET] A2UI Dynamic Schema renders team roster surface", async ({
  page,
}) => {
  await page.goto("/microsoft-agent-framework-dotnet/feature/a2ui_dynamic_schema");
