...
test("[Microsoft Agent Framework .NET] A2UI recovery — invalid render recovers to a valid surface", async ({
  page,
}) => {
  await page.goto("/microsoft-agent-framework-dotnet/feature/a2ui_recovery");
...
test("[Microsoft Agent Framework .NET] A2UI recovery — exhaustion never paints a faulty surface, chat stays usable", async ({
  page,
}) => {
  await page.goto("/microsoft-agent-framework-dotnet/feature/a2ui_recovery");
