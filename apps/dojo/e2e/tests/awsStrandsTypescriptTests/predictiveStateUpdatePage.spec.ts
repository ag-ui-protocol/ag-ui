...
// Predictive state updates for AWS Strands (TypeScript). `write_document` is a
...
//
// The rejection test reads the editor as well, and that is the assertion that
// caught a defect in the shared page: a run ends while a proposal waits on the
// user, and the page used to adopt the diff on screen as the user's own text.
// Reading that markup flattens it, so a struck-out name beside an inserted one
// became a single welded word, which then became both the stored document and
// what a rejection restored.
//
...
const PAGE_URL = "/aws-strands-typescript/feature/predictive_state_updates";
const FIRST_REQUEST =
...
test.describe("Predictive State Updates Feature", () => {
  test("[StrandsTS] maps streaming tool arguments onto document state", async ({
    page,
  }) => {
  ...
  test("[StrandsTS] carries a rejected edit back to the agent", async ({
    page,
  }) => {
    const predictive = new PredictiveStateUpdatesPage(page);
    ...
    // A rejected edit leaves the document exactly as it was. Both halves matter:
    // the new name must be gone, and the old one must be intact rather than
    // fused to it, which is what a flattened diff produces.
    const settled = await predictive.documentText();
    expect(settled).toContain("Atlantis");
    expect(settled).not.toContain("Lola");
    expect(settled).not.toContain("AtlantisLola");
  });
});
