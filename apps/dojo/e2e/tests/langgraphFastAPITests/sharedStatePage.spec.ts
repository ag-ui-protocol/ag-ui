...
test.describe("Shared State Feature", () => {
  ...
  test("[LangGraph FastAPI] should share state between UI and chat", async ({
    ...
    await retryOnAIFailure(async () => {
      ...
      await expect(sharedStateAgent.agentMessage.getByText(/Potatoes/i)).toBeVisible();
      await expect(sharedStateAgent.agentMessage.getByText(/12/)).toBeVisible();
      await expect(sharedStateAgent.agentMessage.getByText(/Carrots/i)).toBeVisible();
      await expect(sharedStateAgent.agentMessage.getByText(/All-Purpose Flour|all.purpose flour/i)).toBeVisible();
    });
  });
});
