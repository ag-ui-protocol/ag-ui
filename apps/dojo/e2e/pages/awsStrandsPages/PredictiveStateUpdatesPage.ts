...
export class PredictiveStateUpdatesPage {
  ...
  /**
   * The editor's text as the user sees it.
   *
   * Plain text on purpose: a diff still on screen flattens into it, which is how
   * a rejection that failed to restore the original is caught.
   */
  async documentText(): Promise<string> {
    return (
      await this.page.locator("div.tiptap.ProseMirror").innerText()
    ).trim();
  }
