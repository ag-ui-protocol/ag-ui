...
export async function setupLLMock(): Promise<void> {
    ...
    match: {
      predicate: (req: { messages: MsgArray; tools?: any[] }) => {
        ...
        const hasLangGraphTool = req.tools?.some(
          (t: any) => t.function?.name === "plan_execution_steps",
        );
        return (
          !!hasLangGraphTool &&
  ...
  const sysContent = (msgs: MsgArray) =>
  ...
  const sysIncludes = (msgs: MsgArray, substr: string) => {
    const sys =
  ...
  mockServer.addFixture({
    match: {
      predicate: (req: { messages: MsgArray }) =>
  ...
  mockServer.addFixture({
    match: {
      predicate: (req: { messages: MsgArray }) =>
        sysContent(req.messages).includes("You are the experiences agent"),
    },
    response: {
      content:
    ...
    match: {
      predicate: (req: { messages: MsgArray; tools?: any[] }) => {
        ...
        const hasStrandsTool = req.tools?.some(
          (t: any) => t.function?.name === "plan_task_steps",
        );
        return (
          !!hasStrandsTool &&
    ...
    match: {
      predicate: (req: { messages: MsgArray; tools?: any[] }) => {
        ...
        const hasPydanticTool = req.tools?.some(
          (t: any) => t.function?.name === "display_recipe",
        );
        return (
          !!hasPydanticTool &&
          textOf(lastUser?.content).includes("pasta recipe")
  ...
  mockServer.addFixture({
    match: {
      predicate: (req: { messages: MsgArray; tools?: any[] }) => {
        const lastUser = req.messages.filter((m) => m.role === "user").pop();
        const hasPydanticTool = req.tools?.some(
          (t: any) => t.function?.name === "create_plan",
        );
        return (
          !!hasPydanticTool &&
          textOf(lastUser?.content).includes("plan to make brownies")
    ...
    match: {
      predicate: (req: { messages: MsgArray; tools?: any[] }) => {
        ...
        const hasLlamaIndexTool = req.tools?.some(
          (t: any) => t.function?.name === "run_task",
        );
        return (
          !!hasLlamaIndexTool &&
          textOf(lastUser?.content).includes("plan to make brownies")
  ...
  mockServer.addFixture({
    match: {
      predicate: (req: { messages: MsgArray; tools?: any[] }) => {
        const lastUser = req.messages.filter((m) => m.role === "user").pop();
        const hasLlamaIndexTool = req.tools?.some(
          (t: any) => t.function?.name === "update_recipe",
        );
        return (
          !!hasLlamaIndexTool &&
          textOf(lastUser?.content).includes("pasta recipe")
  ...
  const toolResultFixture = {
    match: {
      predicate: (req: { messages: Array<{ role: string }> }) => {
  ...
  // Move it to the front (addFixture appends, we need it first)
  const fixtures = (mockServer as any).fixtures as Array<unknown>;
  ...
  mockServer.addFixture({
    match: {
      predicate: (req: {
        ...
        const toolNames =
          req.tools?.map((t: any) => t.function?.name).join(",") ||
          "(no tools)";
  ...
  const allFixtures = (mockServer as any).fixtures as Array<{ match: any }>;
