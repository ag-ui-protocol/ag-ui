...
`RunFinished` has two outcomes:

- `outcome: "success"` — the run completed normally. Carries an optional `result`.
- `outcome: "interrupt"` — the run paused for human input. Carries a non-empty
  `interrupts[]` array; the client resumes by starting a new run whose
  `RunAgentInput` includes a `resume` array addressing every open interrupt.
