...
or function that is currently executing. The optional `description` conveys
what the step *does* in human-readable form, letting UIs render something
meaningful when the `stepName` is a machine-level identifier.
...
`StepStarted` event to properly pair the beginning and end of the step. An
optional `description` may be supplied on completion to summarise what the
step did or report a final status in human-readable form.
