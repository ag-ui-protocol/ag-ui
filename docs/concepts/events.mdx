...
`messageId` must not be reused by a text or reasoning message, and vice versa.
Those carry different shapes of `content` — a structured object for activity, a
string for text and reasoning — so a shared id has no coherent meaning. A client
that receives a text or reasoning message under an id an activity message
already holds should leave the activity message untouched rather than overwrite
it.
