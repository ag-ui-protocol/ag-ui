...
`InputContent` values let user messages mix text with binary payloads (for
images, audio, PDFs, etc.). Text entries keep backwards compatibility with
string-only messages, while binary entries describe rich media with metadata:

- `mimeType` specifies how downstream systems should interpret the payload.
- `data` contains inline base64-encoded bytes for immediate consumption.
- `url` references externally hosted content when direct upload is preferred.
- `id` links to pre-uploaded assets managed elsewhere in the system.
- `filename` provides optional human-readable naming for downloads.

At least one of `data`, `url`, or `id` must be present on binary items so the
agent (or its backing provider) can retrieve the payload.
...
interface BaseMessage {
  ...
  content?: string | InputContent[] // Text or multimodal content of the message
...
interface UserMessage {
  id: string
  role: "user"
  content: string | InputContent[] // User-provided text or multimodal payload
  name?: string // Optional user identifier
}

> **Validation rule:** a user message must include either non-empty string content or a non-empty array of `InputContent`. Binary items must specify at least one of `data`, `url`, or `id` to reference the payload.
```
