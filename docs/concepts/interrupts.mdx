...
sequenceDiagram
  ...
  Agent-->>Client: RunFinished { outcome: "interrupt", interrupts: [...] }
  ...
  Agent-->>Client: RunFinished { outcome: "success", result }
```

## Run outcomes

`RunFinished` carries an `outcome` field with two values:

- `"success"` — the run completed normally. Carries an optional `result`.
- `"interrupt"` — the run paused for user input. Carries a non-empty
  `interrupts[]` array.
