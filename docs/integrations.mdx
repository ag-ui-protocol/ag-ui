...
- **[chanx](https://github.com/huynguyengl99/chanx)** - Typed WebSocket layer for Django Channels and FastAPI/Starlette. Its [AG-UI kit](https://huynguyengl99.github.io/chanx-kit/kits/ag-ui/) carries AG-UI over a WebSocket instead of HTTP+SSE — a transport rather than an agent framework, so anything emitting AG-UI can sit behind it: bidirectional on one connection, with an explicit cancel message and a single run streamed to several browser tabs at once. See a [runnable demo](https://github.com/huynguyengl99/pydantic-ai-ws-agent).
