...
data or a URL. The tool message the event mints has the identical shape, so a
result travels into the next run's `messages` unchanged.
...
- A producer handed a part its model cannot take — an image to a text-only
  model, audio to a provider that accepts none in a tool result — MUST NOT fail
