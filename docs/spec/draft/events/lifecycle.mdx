...
[versioning rules](/spec/draft/basic/versioning#version-negotiation); and MAY
...
- Its `outcome` distinguishes a run that completed from one interrupted
  ...
  cancelled outcome is specified [below](#cancelled-runs).
...
- Everything that holds for a closed run holds here. A producer MUST close
  ...
  [tool call rules](/spec/draft/events/tool-calls#tool_call_end). A producer
...
[truncated run](/spec/draft/basic/transports#truncation), not a cancelled one:
...
cost, one [`TokenUsage`](/spec/draft/schema#tokenusage) entry per provider and
...
- Usage covers every model call made within the run, including calls made by
  its [subagents](/spec/draft/events/subagents): subagent events carry no
...
- A run that [resumes](/spec/draft/basic/patterns/interrupt-resume) an
