...
- A producer MUST NOT report an interrupted run as success: the interrupt
  ...
  messages. A run that stopped because it was *told to* is neither: it ends
  with the [cancelled outcome](/spec/draft/events/lifecycle#cancelled-runs)
...
- Token usage does not carry across the gap: the resuming run's `usage`
  [covers](/spec/draft/events/lifecycle#token-usage) only the model calls it
