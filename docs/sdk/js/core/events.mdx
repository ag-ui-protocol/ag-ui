...
schema validator. Consumers that need to validate untrusted event streams (for
example, when parsing wire data over HTTP) plug in an `AgentValidator` — a pluggable
interface based on [Standard Schema](https://github.com/standard-schema/standard-schema).

The package ships a minimal default validator that performs type-tag checking and
applies the protocol's field defaults:

```typescript
import { defaultEventValidator, EventType } from "@ag-ui/core";
...
For stricter validation you can supply your own validator backed by zod, valibot,
arktype, or any library that implements the Standard Schema spec:
...
both accept an optional validator argument and fall back to `defaultEventValidator`
