...
import io.workm8.agui4j.http.BaseHttpClient; // choose an implementation, e.g., OkHttp
...
HttpAgent agent = HttpAgent.builder()
    .agentId("my-agent")
    .threadId("thread-123")
    .httpClient(myHttpClient) // e.g., new io.workm8.agui4j.okhttp.HttpClient("https://api.example.com/agent")
    .state(new State())
    .build();
